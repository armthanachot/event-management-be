...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.Credit = field.NewFloat64(tableName, "credit")
	_user.Role = field.NewString(tableName, "role")
	_user.Events = userHasManyEvents{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Events", "entity.Event"),
		Organizer: struct {
			field.RelationField
			Events struct {
				field.RelationField
			...
			RelationField: field.NewRelation("Events.Organizer", "entity.User"),
			Events: struct {
				field.RelationField
			}{
...
func (u *user) updateTableName(table string) *user {
	...
	u.Credit = field.NewFloat64(table, "credit")
...
func (u user) clone(db *gorm.DB) user {
	...
	u.Events.db = db.Session(&gorm.Session{Initialized: true})
	u.Events.db.Statement.ConnPool = db.Statement.ConnPool
	return u
}

func (u user) replaceDB(db *gorm.DB) user {
	u.userDo.ReplaceDB(db)
	u.Events.db = db.Session(&gorm.Session{})
	return u
}
...
func (a userHasManyEvents) Model(m *entity.User) *userHasManyEventsTx {
	return &userHasManyEventsTx{a.db.Model(m).Association(a.Name())}
}

func (a userHasManyEvents) Unscoped() *userHasManyEvents {
	a.db = a.db.Unscoped()
	return &a
}
...
func (a userHasManyEventsTx) Find() (result []*entity.Event, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyEventsTx) Append(values ...*entity.Event) (err error) {
...
func (a userHasManyEventsTx) Replace(values ...*entity.Event) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyEventsTx) Delete(values ...*entity.Event) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyEventsTx) Unscoped() *userHasManyEventsTx {
	a.tx = a.tx.Unscoped()
	return &a
}
...
type IUserDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []entity.User, err error)
}

// SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (u userDo) FilterWithNameAndRole(name string, role string) (result []entity.User, err error) {
